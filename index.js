...
// Usamos PORT configurable por entorno (EasyPanel), por defecto 8080
...
// --- CONSTANTE GLOBAL PARA EL FACTOR DE CRUCE (Sacada de tu código) ---
// Esta constante elimina la necesidad de una carga asíncrona crítica al inicio
...
async function getSheetData(range) {
    // Nota: La autenticación se realiza en cada llamada (como en tu código original)
    ...
    try {
        ...
        if (range === RANGO_TASAS && Array.isArray(data) && data.length > 0) {
            const latestRow = data.reduce((max, current) => {
                ...
                // Asegura que solo se devuelva el IDTAS más alto
        ...
        // MUY IMPORTANTE: Registramos el error sin hacer 'throw' si es posible, para mantener el servidor vivo
        console.error(`\n--- ERROR AL LEER GOOGLE SHEETS (${range}) ---`);
        console.error(`DETALLE: ${err.message || 'Error desconocido'}`);
        console.error('--- Las peticiones fallarán con 500/503 si el error no es temporal --- \n');
        // Lanzamos el error para que la ruta de la API pueda manejar el 500
        throw err; 
    }
}
...
// 1. Ruta de Datos Dinámicos (Tasas)
...
// 2. Ruta de Matriz de Cruce Estática (Lectura de Sheets)
app.get('/matriz_cruce', async (req, res) => {
    try {
        // Rango de la Matriz de Cruce en tu sheet
...
// 3. Ruta de Matriz de Emojis/Factores Estática (Lectura de Sheets)
app.get('/matriz_emojis', async (req, res) => {
    try {
        // Rango de la Matriz de Emojis/Factores en tu sheet
...
app.get('/convertir', async (req, res) => {
    ...
    try {
        // 2. OBTENER ÚLTIMA FILA: getSheetData ya devuelve [latestRow]
        const latestRowArray = await getSheetData(RANGO_TASAS); 
        
        if (!Array.isArray(latestRowArray) || latestRowArray.length === 0) {
             return res.status(503).json({ error: "No se pudieron obtener datos de tasas dinámicas recientes. Verifique las credenciales." });
        }
        ...
        if (!T_O_str || !T_D_str) {
             return res.status(404).json({ error: `Clave de tasa no encontrada en Sheets. Verifique que ${Tasa_O_key} y ${Tasa_D_key} existan.` });
        }

        const T_O = parseFactor(T_O_str);
        const T_D = parseFactor(T_D_str);

        if (T_O === 0 || T_D === 0) {
            return res.status(404).json({ error: "El valor de una de las tasas dinámicas es cero o inválido." });
        }

        // 4. BUSCAR FACTOR DE GANANCIA (F) en la matriz fija (MATRIZ_CRUCE_FACTORES)
...
app.listen(PORT, () => {
    ...
    process.on('SIGTERM', () => {
        console.log('[SHUTDOWN] Señal SIGTERM recibida. Cerrando servidor.');
        process.exit(0);
    });
});
